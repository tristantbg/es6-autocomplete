...
class AutoComplete {
  ...
  _addEventListeners() {
    ...
    this.element.addEventListener('click', evt => this._onClick(evt));
  ...
  _onClick(evt) {
    ...
    const itemClicked = evt.target.closest(`.${this.baseClassName}__item`)
    console.log(itemClicked)
    // const itemClicked = evt.target.classList.contains(`${this.baseClassName}__item`);
    if (itemClicked) {
      this._useSelected(itemClicked);
    }
  }
  ...
  _useSelected(element) {
    ...
    const href = selectedItem.getAttribute('data-autocomplete-url')
    if (href) {
      window.location.href = selectedItem.getAttribute('data-autocomplete-url');
    } else {
      this._onBlur()
    }
   ...
   * @returns {string}
   */
  _getRenderListItem(item, url, index, query) {
